...
//        {
//	        var p = GetUniformPersentAtT(i/(float) lineSteps);
//            Vector3 lineEnd = GetPointAt(p);
//	        Handles.DrawLine(lineEnd, lineEnd + Vector3.one); //0.2f*GetDirection(i / (float)lineSteps));
//        }
	...
	// needs testing
	public float GetUniformPersentAtT(float percent)
	{
		...
		{
			var currentPosition = GetPointAt(1f*i/resolution);
			currentLength += (currentPosition - lastPosition).magnitude;
			lastPosition = currentPosition;
			if (currentLength > l)
			{
				for (int j = 1; j <= resolution; j++)
				{
					currentPosition = GetPointAt(1f*i/resolution - j/(2f * resolution));
					currentLength -= (currentPosition - lastPosition).magnitude;
					lastPosition = currentPosition;
					if (currentLength <= l)
					{
						return 1f*i/resolution - j / (2f * resolution);
					}
				}
				return 1f*i/resolution;
			}
		}
		return 1f;
	}
    ...
    private static Vector3 GetQuadraticDerivative(Vector3 p0, Vector3 p1, Vector3 p2, float t)
    {
        return
            2f * (1f - t) * (p1 - p0) +
            2f * t * (p2 - p1);
    }
    private static Vector3 GetCubicDerivative(Vector3 p0, Vector3 p1, Vector3 p2, Vector3 p3, float t)
    {
        //t = Mathf.Clamp01(t);

        //Vector3 part1 = 3 * Mathf.Pow(1 - t, 2) * p0;
        //Vector3 part2 = 3 *  (1 - 4*t + 3*t*t) * p1;
        //Vector3 part3 = 3 * (2*t - 3*t*t)* p2;
        //Vector3 part4 = 3 * Mathf.Pow(t, 2) * p3;
    ...
    public Vector3 GetVelocity(float t)
    {
        ...
        for (int i = 0; i < points.Length - 1; i++)
        {
            curvePercent = ApproximateLength(points[i], points[i + 1], 50) / length;
            if (totalPercent + curvePercent > t)
            {
                p1 = points[i];
                p2 = points[i + 1];
        ...
        if (p1.handle2 != Vector3.zero)
        {
            if (p2.handle1 != Vector3.zero) result = GetCubicDerivative(p1.position, p1.globalHandle2, p2.globalHandle1, p2.position, curvePercent);
            else result = GetQuadraticDerivative(p1.position, p1.globalHandle2, p2.position, curvePercent);
        }
        else
        {
            if( p2.handle1 != Vector3.zero )
                result = GetQuadraticDerivative(p1.position, p2.globalHandle1, p2.position, curvePercent);
            else result = p2.position - p2.position;
        }
		

        result = GetCubicDerivative(p1.position, p1.globalHandle2, p2.globalHandle1, p2.position, curvePercent);
        //return 0.2f*(transform.InverseTransformDirection(result));transform.TransformPoint
        return (0.2f * (result));
	    ...
	    */
	    ...
	    return transform.TransformPoint(GetCubicDerivative(p1.position, p1.globalHandle2, p2.globalHandle1, p2.position, t)) - transform.position;
    }
